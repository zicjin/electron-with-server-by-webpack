...
const profileModel: ProfileModel = {
  ...
  setSettings: action((state, { settings, storage = true, post = true }) => {
    ...
    if (storage) {
      send('setStore', { settings: state.settings })
    }
  ...
  setProfile: action((state, { profile, storage = true }) => {
    ...
    if (storage) {
      send('setStore', { profile })
    }
  }),

  load: thunk(async (actions, payload, { getState }) => {
    const localSettings = await send('getStore', 'settings')
    if (localSettings) {
      actions.setSettings({
    ...
    const localJwt = await send('getStore', 'jwt')
    if (localJwt) {
      actions.setJwt({ jwt: localJwt, storage: false })
    }

    const localProfiles = await send('getStore', 'profile')
    if (localProfiles) {
